...
const InsertRoutine = ({ setControl, control }) => {
  ...
  const handleInsertRoutineFunc = (form) => {
    ...
    console.log(form);
    axios.post(`${url}/routine/insert-routine`).then(res => {
    ...
    <dialog id="insertRoutine" className="modal">
      <div className="modal-box">
        <form onSubmit={handleSubmit(handleInsertRoutineFunc)} className="space-y-2 w-full">
          ...
          {/* Row index */}
          <div className=''>
            <label htmlFor="rowIndex" className="block mb-2 text-sm font-medium text-slate-500 dark:text-white">Row index</label>
            <select id='rowIndex' className='my-inp' {...register("rowIndex", { required: true })}>
            ...
            {errors.rowIndex && <p className="text-red-500">*This field is required</p>}
          </div>
